...
class Request {
	...
	public function redirect($url){
		header('Location: '.config('app')['base_url'].'/'.$url);
	}
	...
	public static function view($viewFile, $data = null, $renderable = true) {
		...
		$actualFile = 'src/views/' . $viewFile . ".php";
		ob_start();
        ...
        $output = str_replace("~", config('app')['base_url'] . '/public/', $output);
        $output = str_replace("url:", config('app')['base_url'] ."/", $output);
