...
class Router extends Error_Handling {
    ...
    public static function extend($routeFileName){
        $dir = rtrim(__DIR__,'system/Routing');
        require $dir.'/src/'.$routeFileName.'.php';
    }
    ...
    public static function run($url) {
        ...
        echo Error_Handling::display(404);
        exit;
    }
    ...
    private static function handleString($url) {
        ...
        if (count($args) > 1) {
            $controllerName = '\\Src\\Controllers\\' . $args[0];
            $methodName = $args[1];
